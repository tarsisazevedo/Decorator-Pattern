...
from decorator_pattern.decorators.decorator_creme import *
...
class PedindoUmCafeComAdicional(Historia):
    ...
    """
    cenarios=['PedindoUmCafeAModaDaCasaComWhip', 'PedindoUmCafeAModaDaCasaComLeite', 'PedindoUmCafeAModaDaCasaComCreme']
    colored = True
...
class PedindoUmCafeAModaDaCasaComCreme(Cenario):
    DadoQue("Eu sou um cliente")
    DadoQue("e eu quero comprar um cafe a moda da casa")
    @DadoQue("eu quero acrescentar creme no meu cafe")
    def cafe_com_creme(self):
        self.cafe_com_creme = Creme(self.cafe_a_moda_da_casa)
    
    @Quando("eu peço meu café, que custa 0.99, com creme, que custa 0.20")
    def pedir_cafe_com_creme(self):
        self.cliente.pedir_cafe(self.cafe_com_creme)

    @Entao("eu tenho meu cafe com creme e uma conta de 1.19")
    def eu_tenho_uma_conta(self):
        self.cliente.get_conta() |should_be.equal_to| 1.19    
